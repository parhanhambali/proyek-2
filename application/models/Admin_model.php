...
class Admin_model extends CI_Model
{
    ...
    public function getBarang()
    {
        ...
        $this->db->join('size z', 'b.size_id = j.id_size');
        $this->db->join('bahan h', 'b.bahan_id = s.id_bahan');
        $this->db->join('warna w', 'b.warna_id = j.id_warna');
        return $this->db->get('barang b')->result_array();
    }
    ...
    public function getHargaawal($limit = null, $id_barang = null, $range = null)
    {
        $this->db->select('*');
        $this->db->join('user u', 'bm.user_id = u.id_user');
        $this->db->join('supplier sp', 'bm.supplier_id = sp.id_supplier');
        ...
        $this->db->order_by('id_harga_awal', 'DESC');
        return $this->db->get('harga_awal bm')->result_array();
    }

    
    public function getHargapromo($limit = null, $id_barang = null, $range = null)
    {
        $this->db->select('*');
        $this->db->join('user u', 'bk.user_id = u.id_user');
        $this->db->join('barang b', 'bk.barang_id = b.id_barang');
        $this->db->join('mata s', 'b.mata_id = s.id_mata');
        if ($limit != null) {
            $this->db->limit($limit);
        }
        if ($id_barang != null) {
        ...
        $this->db->order_by('id_harga_promo', 'DESC');
        return $this->db->get('harga_promo bk')->result_array();
    }
